...
int main(int argc, char* argv[]) {
	...
	if (dumpBinary) {
		cout << "binary->" << endl;
		cout << "------------------------------------------------------------" << endl;
		for (auto it = dump.sectionDump.begin(); it != dump.sectionDump.end(); ++it) {
			cout << "section: " << it->name << endl;
			for (auto si = it->data.begin(); si != it->data.end();) {
				cout << "    ";
				for (int i = 0; si != it->data.end() && i++ < 4; ++si)
					printf("%02X ", *si);
				cout << endl;
			}
		}
